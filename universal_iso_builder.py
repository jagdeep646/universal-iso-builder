...
- Actual ISO creation requires one local backend tool:
...
class IsoBuilderApp(tk.Tk):
    ...
    def refresh_backends(self) -> None:
        ...
        if not self.detected_backends:
            ...
            self.log("Windows: Windows ADK Deployment Tools install karke oscdimg use karo.")
    ...
    def get_selected_backend(self) -> Backend:
        if not self.detected_backends:
            raise RuntimeError("No ISO backend found. Install oscdimg/xorriso/genisoimage/mkisofs or use macOS hdiutil.")
