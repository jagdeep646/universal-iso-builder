...
class IsoBuilderApp(tk.Tk):
    ...
    def _build_worker(
        ...
        try:
            self.after(0, lambda: self._set_status("Build started", f"Using backend: {backend.name}"))
            self.thread_log("=" * 72)
            ...
            if self.dry_run_var.get():
                ...
                self.after(0, lambda: self._set_status("Dry run finished", f"Output preview: {output_iso.name}"))
                return
            ...
            self.after(0, lambda: self._set_status("Build finished: PASS", f"Package folder ready: {output_iso.parent}"))
        except Exception as e:
            self.thread_log(f"ERROR: {e}")
            self.thread_log("Build finished: FAIL")
            self.after(0, lambda: self._set_status("Build finished: FAIL", str(e)))
        finally:
