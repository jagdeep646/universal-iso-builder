# Universal ISO Builder - isolated PySide6/Qt Quick ONEDIR build.
# This preview build does not replace the verified Tkinter release output.
...
$BuildRoot = Join-Path $BuildBase "$AppName Qt"
$WorkPath = Join-Path $BuildRoot "work"
$SpecPath = Join-Path $BuildRoot "spec"
$DistRoot = Join-Path $ScriptRoot "dist-qt"
...
function Remove-ScopedDirectory {
    ...
    if (!$targetPath.StartsWith($rootPath, [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "Refusing cleanup outside expected Qt build root: $targetPath"
    }
...
Write-Host ""
Write-Host "============================================================"
Write-Host "Building isolated Qt ONEDIR preview: $AppName v$AppVersion"
...
foreach ($requiredFile in @($AppScript, $GuiRequirements, $VersionFile)) {
    if (!(Test-Path -LiteralPath $requiredFile -PathType Leaf)) {
        throw "Required Qt build file not found: $requiredFile"
    }
}
...
& $PythonExe -m PyInstaller `
  ...
  --name "$AppName" `
  "$AppScript"
Assert-NativeCommandSucceeded -Step "Qt PyInstaller EXE build" -ExitCode $LASTEXITCODE
...
if (!(Test-Path -LiteralPath $ExpectedExe -PathType Leaf)) {
    throw "PyInstaller reported success but expected Qt EXE was not found: $ExpectedExe"
}
if (!(Test-Path -LiteralPath $ExpectedQml -PathType Leaf)) {
    throw "Qt build is missing packaged QML assets: $ExpectedQml"
}

Write-Host ""
Write-Host "============================================================"
Write-Host "QT ONEDIR BUILD PASS"
...
Write-Host ""
Write-Host "Share/test the complete folder, including _internal."
Write-Host "The existing Tkinter dist output was not replaced."
Write-Host "============================================================"
Write-Host ""
