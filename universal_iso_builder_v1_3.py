...
class IsoBuilderApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Checkbutton(checks, text="Auto name + package folder", variable=self.auto_package_var).pack(side="left", padx=(0, 18))
    ...
    def validate_paths(self) -> Tuple[Path, Path, str, str]:
        ...
        if self.auto_package_var.get():
            ...
            # Keep UI visible fields synced with the actual auto-generated names.
            self.iso_name_var.set(iso_name)
            self.label_var.set(label)
        else:
